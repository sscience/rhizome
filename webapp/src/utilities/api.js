...
function emptyResponsePoint(path, mode, defaultVersion, useDefaults) {
  ...
  fetch.toString = function(query, version) {
    ...
    return (
      BASE_URL + versionedPath + urlencode(_.defaults({}, query, defaults))
    );
  };
...
function submissionDownload(document_id) {
  ...
  return new Promise(function(fulfill, reject) {
    fetch({ document_id: document_id, format: "csv" }, null, {
  ...
  return new Promise(function(fulfill, reject) {
    ...
    fetch(q, null, { "cache-control": "no-cache" }).then(function(data) {
      ...
      for (var i = data.objects.length - 1; i >= 0; --i) {
        data.objects[i].campaign = setCampaign(
          campaignIx[data.objects[i].campaign]
        );
      }
      fulfill(data);
    }, reject);
...
function buildIndicatorsTree(
  ...
  _.each(sortTags, function(t) {
    ...
    t.noValue = true;
    t.parent =
      t.parent_tag_id && t.parent_tag_id !== "None"
        ? makeTagId(t.parent_tag_id)
        : null;
    t.children = [];
    t.title = t.tag_name;
    t.value = t.id;
...
export default {
  ...
  remove_calc_from_indicator: emptyResponsePoint(
  ...
  delete_location_responsibility: emptyResponsePoint(
    "/location_responsibility",
    "delete",
    1,
    false
  ),
  remove_indicator_from_tag: emptyResponsePoint(
    "/indicator_to_tag",
    "delete",
    1,
    false
